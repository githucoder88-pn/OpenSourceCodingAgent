...
impl OllamaProvider { pub fn new(base_url: Option<String>) -> Self { Self { base_url: base_url.unwrap_or_else(|| "http://localhost:11434".into()) } } }
#[async_trait]
impl ModelProvider for OllamaProvider {
    fn name(&self) -> &str { "ollama" }
    fn models(&self) -> Vec<String> { vec!["llama3.1".into(), "mistral".into(), "codellama".into(), "qwen2.5-coder".into()] }
    ...
    async fn generate(&self, _r: ModelRequest) -> Result<ModelResponse, ModelError> {
        Ok(ModelResponse { id: uuid::Uuid::new_v4().to_string(), model: "llama3.1".into(), choices: vec![Choice { index: 0, message: Message { role: "assistant".into(), content: "Hello from Ollama stub (local)".into(), tool_calls: None, tool_call_id: None }, finish_reason: Some("stop".into()) }], usage: Some(Usage { prompt_tokens: 10, completion_tokens: 20, total_tokens: 30 }) })
    }
    async fn stream(&self, _r: ModelRequest) -> Result<tokio::sync::mpsc::Receiver<Result<StreamChunk, ModelError>>, ModelError> {
        let (tx, rx) = tokio::sync::mpsc::channel(100);
        tokio::spawn(async move { let _ = tx.send(Ok(StreamChunk { delta: "Hello from Ollama".into(), tool_calls: None, finish_reason: Some("stop".into()) })).await; });
        Ok(rx)
    }
